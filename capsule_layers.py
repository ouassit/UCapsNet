...
class DeconvCapsuleLayer(layers.Layer):
    ...
    def call(self, input_tensor, training=None):
        ...
        if self.upsamp_type == 'resize':
            ...
            output_padding = None
            out_height = deconv_length(self.input_height, self.scaling, self.kernel_size, self.padding, output_padding)
            out_width = deconv_length(self.input_width, self.scaling, self.kernel_size, self.padding, output_padding)
    ...
    def compute_output_shape(self, input_shape):
        ...
        output_padding = None
        output_shape[1] = deconv_length(output_shape[1], self.scaling, self.kernel_size, self.padding, output_padding)
        output_shape[2] = deconv_length(output_shape[2], self.scaling, self.kernel_size, self.padding, output_padding)
